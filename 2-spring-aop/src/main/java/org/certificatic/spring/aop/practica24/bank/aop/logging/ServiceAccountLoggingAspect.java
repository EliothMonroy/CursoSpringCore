...
public class ServiceAccountLoggingAspect implements Ordered {
	...
	@Before("org.certificatic.spring.aop.practica24.bank.aop."
			+ "PointcutDefinition.serviceLayer() && args(account, ..)")
	public void beforeServiceAccountMethodExecutionAccount(JoinPoint jp, Account account) {
		...
		log.info("{}",
				colorWriter.getColoredMessage(Color.MAGENTA,
						String.format("In %s.%s(), Logging Service Account access. Account: %s",
								clazz, method, account.getAccountNumber())));
	}

	// Define Advice Before que intercepte serviceLayer() y cache los argumentos
	@Before("org.certificatic.spring.aop.practica24.bank.aop."
			+ "PointcutDefinition.serviceLayer() && args(customerId, ..)")
	public void beforeServiceAccountMethodExecutionLong(JoinPoint jp, Long customerId) {
		...
		log.info("{}",
				colorWriter.getColoredMessage(Color.MAGENTA,
						String.format("In %s.%s(), Logging Service Account access. Customer Id: %s",
								clazz, method, customerId)));
